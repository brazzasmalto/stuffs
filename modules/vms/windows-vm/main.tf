...
resource "azurerm_public_ip" "trustee" {
...
resource "azurerm_network_interface" "trustee" {
  name                = "${var.name}-nic"
  resource_group_name = var.resource_group_name
  location            = var.location
  tags                = var.tags
  ...
  ip_configuration {
    ...
    public_ip_address_id          = var.public_ip ? azurerm_public_ip.trustee["pip"].id : null
  }
}

resource "azurerm_network_interface_application_security_group_association" "trustee" {
  network_interface_id          = azurerm_network_interface.trustee.id
  application_security_group_id = var.asg_id
}

resource "azurerm_windows_virtual_machine" "trustee" {
  name                = var.name
  resource_group_name = var.resource_group_name
  location            = var.location
  tags                = var.tags
  ...
  network_interface_ids = [azurerm_network_interface.trustee.id]
