...
resource "random_password" "trustee" {
...
###
resource "azurerm_postgresql_server" "trustee" {
  name                              = format("%s", var.postgresql_server_name)
  resource_group_name               = local.resource_group_name
  location                          = local.location
  administrator_login               = var.admin_username == null ? "postgresadmin" : var.admin_username
  administrator_login_password      = var.admin_password == null ? random_password.trustee.0.result : var.admin_password
...
###
resource "azurerm_postgresql_database" "trustee" {
  name                = var.postgresql_server_settings.database_name
  resource_group_name = local.resource_group_name
  server_name         = azurerm_postgresql_server.trustee.name
...
###
resource "azurerm_postgresql_configuration" "trustee" {
...
###
resource "azurerm_postgresql_firewall_rule" "trustee" {
...
###
resource "azurerm_postgresql_active_directory_administrator" "trustee" {
...
###
resource "azurerm_postgresql_server_key" "trustee" {
  count            = var.key_vault_key_id != null ? 1 : 0
  server_id        = azurerm_postgresql_server.trustee.id
...
###
resource "azurerm_postgresql_virtual_network_rule" "trustee" {
...
###
# Private Link for PostgreSQL Server - Default is "false"
###
...
resource "azurerm_private_endpoint" "pep1" {
  count               = var.enable_private_endpoint ? 1 : 0
  name                = format("%s-private-endpoint", var.postgresql_server_name)
  location            = local.location
  resource_group_name = local.resource_group_name
  subnet_id           = azurerm_subnet.snet-ep.0.id
  tags                = merge({ "Name" = format("%s-private-endpoint", var.postgresql_server_name) }, var.tags, )

  private_service_connection {
    name                           = "postgresqldbprivatelink"
    is_manual_connection           = false
    private_connection_resource_id = azurerm_postgresql_server.trustee.id
...
data "azurerm_private_endpoint_connection" "private-ip1" {
  ...
  resource_group_name = local.resource_group_name
  depends_on          = [azurerm_postgresql_server.trustee]
}
...
resource "azurerm_private_dns_a_record" "arecord1" {
  count               = var.enable_private_endpoint ? 1 : 0
  name                = azurerm_postgresql_server.trustee.name
...
resource "azurerm_monitor_diagnostic_setting" "extaudit" {
  ...
  target_resource_id         = azurerm_postgresql_server.trustee.id
